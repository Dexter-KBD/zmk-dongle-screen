...
int zmk_widget_mod_status_init(struct zmk_widget_mod_status *widget, lv_obj_t *parent)
{
    ...
    lv_obj_set_size(widget->obj, 180, 140); // 전체 위젯 크기 조정

    // 모디 레이블 (중앙 하단)
    widget->label = lv_label_create(widget->obj);
    lv_obj_align(widget->label, LV_ALIGN_BOTTOM_MID, 0, -10); // 하단 중앙
    ...
    lv_label_set_text(widget->caps_word_label, "󰄛");  // 심볼이나 글자로 변경 가능
    lv_obj_align(widget->caps_word_label, LV_ALIGN_TOP_MID, 0, 0); // 상단 중앙
